function [PHI] = Phi_2_3(argument,psi,Apsi)
%% Phi_2_3 function
...
PSI_0_c = (1/C^2)*(1/12-(gamma^2/2-2*gamma^3/3+gamma^4/4));

PSI_1_c = (1/C)*((1/2)*(1-gamma^2)-(1/3)*(1-gamma^3));
...
PSI_0_d = (1/B^2)*(gamma^2/2+2*gamma^3/3+gamma^4/4);

PSI_1_d = -1/B*(gamma^2/2+gamma^3/3);
...
PSI_0_e = -(1/B^2)*(delta^2/2-2*delta^3/3+delta^4/4);

PSI_1_e = 1/B*(delta^3/3-delta^2/2);
...
PSI_0_f = (1/C^2)*((delta^2/2+2*delta^3/3+delta^4/4)-1/12);

PSI_1_f = 1/C*(1/2*(delta^2-1)+1/3*(delta^3+1));
